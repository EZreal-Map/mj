...
const rowClickCallback = (row) => {
  ...
  ElMessage.success(`已选中年份：${clickedRow.value.Year}对应的月平均流量数据`)
}
...
const getOriginalHydro = async () => {
  const data = await getOriginalHydroAxios(selectSTCDT.value)
...
const addDialogCallback = () => {
  ...
  addMonthlyFlowAxios(selectSTCDT.value, monthFlowForm.value).then(async () => {
    ...
    if (matchRow) {
      ...
      rowClickCallback(matchRow) // 更新左侧表格和图表
    }
  })
}
</script>
